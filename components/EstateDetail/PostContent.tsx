...
const PostContent = (props: IPost) =>
{
    ...
    const RoomSection = () =>{
        if (post.bathroomNumber==0 && post.bedroomNumber==0)
            return null
        const roomString = (post.bathroomNumber>0 && post.bedroomNumber>0) 
            ? `${post.bedroomNumber} PN + ${post.bathroomNumber} PT`
            : (post.bathroomNumber? `${post.bathroomNumber} PT` : `${post.bedroomNumber} PN `)
    ...
    return (
        <div>
            <ImageCarousel imageList={post.images} className="border-2xl overflow-clip rounded-lg"/>
                <div className="p-3">
                    <div>
                        <h1 className="text-2xl font-bold text-black uppercase">{post.title}</h1>
                        {post.publishedDate && <div className='mt-3 flex items-center '>
                            <ClockIcon className='w-5 h-5 inline-block'/>
                            <span className="w-2"></span>
                            {` Ngày đăng: ${post.publishedDate}`}
                            </div>}
                        {post?.address && <div className='mt-1 flex items-center'>
                    ...
                    <div className="flex justify-between">
                        ...
                        <div className="flex-1">
                            <p>Diện tích</p>
                            <p className="font-bold">{`${post.area} m`}<sup>2</sup></p>
                        </div>
                        {RoomSection()}
